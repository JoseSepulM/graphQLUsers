...
public class DeleteUserFunction {
    ...
    @FunctionName("EliminarUserGrapQl")
    public HttpResponseMessage run(
        @HttpTrigger(
            name = "req",
            methods = {HttpMethod.POST},
            authLevel = AuthorizationLevel.ANONYMOUS,
            route = "usuarioGraphQL/eliminar"
        )
        ...
        try {
            ...
            context.getLogger().info("🔵 Body recibido en ELIMINAR: " + body);

            ObjectMapper mapper = new ObjectMapper();
            JsonNode jsonNode = mapper.readTree(body);
            String query = jsonNode.get("query").asText();
            context.getLogger().info("🔵 Query extraída en ELIMINAR: " + query);
            ...
            context.getLogger().severe("Error ejecutando GraphQL para eliminar usuario: " + e.getMessage());
